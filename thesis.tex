...
An entity is just any object that can have data attached as attributes.
...
An entity can also be a \emph{series}.
Every series contains an ordered list of contained entities.
Series can also contain other series, so we can model a hierarchy relation with series contained in other series.
There is always one particular series without a parent, this series is the root node of the hierarchy.
If we only want to display tabular data, we just have one or two levels of hierarchy, for single or multiple data series respectively.
Furthermore, if we want to display arbitrary relations among entities, we can model these as \emph{relations}.
Since every \emph{relation} is a \emph{entity} we can add \emph{attributes} to the relation.
These attributes on a relation can e.g.\ store the weight of an edge in a flow map.
...
Every visual effect of the interaction will be preceded by a change of the respective describing function.
In order to coordinate the effect of an interaction, a view can populate the changed function to all related views.


Based on the examples in section~\ref{sec:analysis:examples} we can describe interactions as follows:
\begin{enumerate}
    \item 
    ...
    \item 
        ...
        The $FilterByAttributes$ returns a arbitrary long, ordered list of names of attributes that are taken as input for the $ FilterByValue $ function.
    \item 
        $ Focus: \varnothing \rightarrow \mathbb{N} $

        The $ Focus $ function returns a single id of the entity that should in the center of the viewport.
    ...
    \item 
        ...
        This sort function will return a continuous value based on a arbitrary long list of pairs of attribute values.
...
Let's have some examples how these functions can be applied to coordinate interactions:

A user clicks on feature in a bar chart and this feature is then highlighted.
To coordinate the highlighting, the bar chart visualization changes the \emph{Highlight} which will return only the id of the highlighted entity of the feature.

When a region in a geographical map is clicked, the \emph{Focus} function will return the id of the clicked entity.
A corresponding tree map can show the subtree with the focused entity as root node.
...
The updated \emph{FilterByValues} function will expect these arguments and return either true or false based on the values of these attributes.
...
E.g.\ a feature of the visualization is changed, e.g.\ a colour of a bar in a bar chart.
...
But a scatter plot connects both x- and y-coordinates of an entitiy to a certain data attribute.
Therefore, not only the \emph{trigger} and \emph{effect} but also the handling of the interaction itself is implementation specific.

Every view decides for its own, whether to react or not to a certain interaction.
So we have distinguishable named interactions, and every view can subscribe to certain interactions.
If a view wants to trigger an interaction it can publish to the named interaction.

This pattern is known as the \emph{Publish-subscribe pattern} and widely used in message queues.
The term \emph{interaction} in our case is equivalent to the termn \emph{channel} or \emph{topic} used in message queues.
