...
\subsection{Visualization catalogue}\label{sec:concept:visualization-catalogue}
...
\begin{figure}
    \begin{center}
        ...
        \label{fig:concept:chart-types:bar-diagrams:interactions}
        {\small
            \begin{tabulary}{\textwidth}{ll}
...
To account for the various data structures, we develop an abstract data structure that is powerful enough to include tabular, hierarchical and relational data.
...
It can have arbitrary many attributes and the attributes can be identified via the name.
...
Every series contains an ordered list of entities.
Because a series can also contain other series, we can model a hierarchy relation with series.
There is always one series without a parent series, this series is the root node of the hierarchy.
...
Based on the examples in section~\ref{sec:concept:visualization-catalogue} we can describe interactions as follows:
\begin{enumerate}
    \item 
    ...
    \item 
        $ ColourAttributes: \mathbb{R} \rightarrow \mathbb{C} $
