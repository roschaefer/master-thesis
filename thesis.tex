...
\subsubsection{Trigger and Effect}
...
Note that this implicit meaning must be explicit in \cmvs{} so that other views are able to understand it.
...
\subsubsection{Interaction subject}
The interaction \emph{type} is not sufficient enough to describe an interaction.
...
Interaction type and interaction subject are interrelated in the same way like the name and the list of arguments of a function signature.
...
Therefore we allow for a user-defined \emph{application context}.
The application context describes the interaction in the context of a task or an intention that is intrinsic to the application the interaction happens in. 

\subsubsection{Message}
A message is the information that is send from one view to another in order to coordinate an interaction.
It contains everything necessary to ensure the interaction is reflected in the other view.
...
\begin{equation}
  I(T, M(AC, IT, F(IS)), E)
\end{equation}
For a trigger $T$, an application context $AC$, an interaction type $IT$, a function definition $F$, an interaction subject $IS$ and the effect $E$.
The part of the interaction which is shared among multiple views is the application context $AC$.
The function definition $F$ is defined by the view at runtime.
...
\subsubsection{Predefined Interaction Types}\label{sec:concept:types}
...
\begin{enumerate}
    ...
    \item
        $ \mathbb{D} : \mathbb{D} \subseteq \mathbb{\sum^*} $
...
The list of all sequences of all entities in $E$ is written as $ \mathbb{E^*} $ and includes all subsets of $E$ with a given order.
The empty set is written as $\varnothing$.
A function with the empty set as its domain corresponds to a function without arguments, also called constant.

The following is a list of functions describing the types based on the aforementioned sets:
\begin{enumerate}
    \item
        $ Select: \varnothing \rightarrow \mathcal{P}(\mathbb{E})$

        The constant $Select$ takes no arguments and returns a subset of selected entities.
    \item
        $ Filter: \mathbb{E} \rightarrow \{ \bot, \top \} $

        The $Filter$ function describes which entities are part of the visualization.
        It can either be implemented as a function which returns \attr{true} or \attr{false} if an entity is part of an already known set.
        It can also be implemented based on the values of the dimensions in $\mathbb{D}$ of the entity.
        A threshold function would be an example of the latter.
    \item
        $ Focus: \varnothing \rightarrow \mathbb{E} $

        The constant function $Focus$ returns a single entity that should be in the center of the viewpoint.
        Different visualizations can have different interpretations of this: 
    ...
    \item
        $ Window: \mathbb{D} \rightarrow \mathbb{R^*} $
        ...
        For charts with continuous values along the x- and y-axes, the function returns the \attr{from} and \attr{to} of the interval, which is an implicitly defined set in $\mathbb{R}$.
        If a dimension has discrete values, this function can also return an explicit set of values.

        Scatter plots, bar charts, line diagrams, bubble charts all have two coordinate axes, so \attr{x} and \attr{y} will each be mapped to a pair of values.
        In geographical visualizations we would map the \attr{latitude}, \attr{longitude} and \attr{altitude} to three values in $\mathbb{R}$.
        In a calendar we map \attr{fromDay}, \attr{toDay}, \attr{fromHour} and \attr{toHour} to define the currently visible time section.
        A special attribute of our shared data model is the height of the subtree of an entity.
    ...
    \item
        $ Order: \mathbb{E} \times \mathbb{E} \rightarrow \mathbb{R} $
    ...
    \item
        $ Encode: \mathbb{D} \rightarrow \mathbb{V} $
        ...
        Bubble charts can encode a different data attribute in the \attr{area} of the bubbles.
...
\begin{figure}[h!]
  ...
  \caption{%
    ...
    This is done by a user-defined application context.
