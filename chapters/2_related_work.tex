...
In this section four types of geographic visualization are introduced, which are commonly used to display both geographic and hierarchic data.
...
These relationships can encode additional data attributes in the size or the colour of the stroke, its direction or shape.
Flow maps often display the flow of goods or the migration of people.
...
\subsection{Coordinated Multiple Views}\label{sec:related-work:cmvs}
...
\textbf{JavaScript and TypeScript}
...
\subsection{Formalization of Interactions}
...
The authors distinguish user-independent \emph{animations} and user-controlled \emph{interactions}.
E.g.\ a transformation may display a particular slice of the cube.
An animation would display one slice at a time and display the next slice every second, whereas the interaction would show the slice determined by a user-controlled slider.

Various transformations and their best use in practice are evaluated in this work.
The work focuses on temporal data or continuous data and interactions depend on the underlying data structure and visualization.
The authors admit ``our framework does not provide much guidance for interaction design: the design space for interactive operations has only been partially explored.''\cite[p.~15]{Bach2014}
...
This statement holds e.g.\ for an interaction in which the user increases the visible level of hierarchy in a tree map, which is an \attr{abstract/elaborate} interaction according to \textcite{Yi2007}.
