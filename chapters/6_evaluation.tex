...
\begin{figure}
  ...
  \qquad
  \subfloat[\gv{}]{{\includegraphics[width=0.4\textwidth]{figures/evaluation/use-cases/1-map} }}%
\end{figure}

\subsection{Multi-Select in \tmap{}}

This use case demonstrates the benefit of a multiple select plus the ability to connect interesting features with their geographical context.
Figure~\ref{fig:evaluation:cases:wahlkreise_multiselect} shows a \tmap{} with the following configuration:
The layout is based on the population density, large cluster at the top are sparsely populated districts.
Colour is based on the increase or decrease of inhabitants, a red colour indicating a decrease of inhabitants.
Height is inversely mapped to unemployment rate, a high block indicating a low unemployment rate.
...
However, without the \gv{} it is not possible to draw the hypothesis of the proximity to a highway being the reason for the high prices.
