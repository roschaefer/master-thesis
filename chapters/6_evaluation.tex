...
\subsection{Multi-Select in \tmap{}}\label{sec:evaluation:use-cases:bounding-box}
...
Similar to the scenario in Section~\ref{sec:evaluation:use-cases:gas-stations}, with the \tmap{} alone we can spot anomalies.
However, we would not be able to relate these anomalies to a geographic context.
This time, we can really benefit of the multiple selection as the white group is scattered across the \tmap{}.
...
\subsection{Bounding Box Selection in \gv{}}
Similar to the multiple select in Section~\ref{sec:evaluation:use-cases:bounding-box}, multiple selects can be carried out by a bounding box in the \gv{}.
...
However, the districts are not in the group with the highest construction activity at the very top.
