...
\section{Implementation of the Conceptual Framework}
...
\subsection{Encoding of category, purpose and subject}
...
This approach gives views more flexibility, because they can either subscribe to all interactions or just a few specific interactions.
...
Data attributes of entities are encoded as properties and a hierarchic structure can be encoded in feature collections.
...
In Figure~\ref{fig:implementation:architecture} views with a \attr{render} are implemented as React components and can be rendered in place of a node inside the \gls{dom} tree.
