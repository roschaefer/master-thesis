...
The \attr{render} is the only required method of a React component.
When the component is rendered in the \attr{DOM} tree of the document, the \attr{render} method will be called and returns the inserted HTML.
React uses a templating language called \attr{JSX}, which can be written right into the javascript method.
This templating language allows to nest other child components in to the React parent component.
In this case we add a \attr{Geojson} component.
